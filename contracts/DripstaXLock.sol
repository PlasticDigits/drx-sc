...
contract DripstaXLock is AccessControlEnumerable {
    ...
    event UpdateBoostedLockWad(
        address account,
        uint256 drxWad,
        uint256 boostedDepositWad
    );
    event Withdraw(
        address account,
        address referrer,
    ...
    function lock(uint256 _wad, string calldata _code) public {
        ...
        address referrer = referralCodeRegistry.isCodeRegistered(_code)
            ? referralCodeRegistry.codeToAccount(_code)
            : projectDev;
    ...
    function initiateWithdrawRequest() public {
        require(
            ...
            "DrxLock: Withdraw request already initiated"
        );
        require(drxLocked[msg.sender] > 0, "DrxLock: No drx locked");
        drxRewardPool.withdrawViaLock(
            msg.sender,
            boostedDepositWad[msg.sender]
        );
    ...
    function withdraw(string calldata _code) public {
        require(
            withdrawInitiationEpoch[msg.sender] > 0,
            "DrxLock: Must initiate withdraw first"
        );
        require(
            withdrawInitiationEpoch[msg.sender] >=
                block.timestamp +
                    boostOptions[selectedBoost[msg.sender]].period,
            "DrxLock: Must wait for boost period to pass"
        );
    ...
    function selectBoost(uint64 _boostIndex) public {
        require(
            drx.firstBuyEpoch(msg.sender) >=
                block.timestamp + periodSinceFirstBuyToLock,
            "DrxLock: Must wait for frist buy lock period"
        );
        require(
            _boostIndex >= selectedBoost[msg.sender] ||
                drxLocked[msg.sender] == 0,
            "DrxLock: Cannot select boost lower than current boost unless no lock"
        );
        require(
            withdrawInitiationEpoch[msg.sender] == 0,
            "DrxLock: Cannot select boost when withdraw request active"
        );
    ...
    function cancelWithdrawRequest() public {
        require(
            withdrawInitiationEpoch[msg.sender] != 0,
            "DrxLock: No withdraw request to cancel"
        );
        withdrawInitiationEpoch[msg.sender] = 0;
    ...
    function _updateBoostedLockWad() internal {
        ...
        uint256 newBoostedDepositWad = (drxLocked[msg.sender] *
            boostOptions[selectedBoost[msg.sender]].boostBps) / 10000;
        drxRewardPool.depositViaLock(msg.sender, newBoostedDepositWad);
        boostedDepositWad[msg.sender] = newBoostedDepositWad;
        emit UpdateBoostedLockWad(
            msg.sender,
            drxLocked[msg.sender],
            newBoostedDepositWad
        );
    }

    function ADMIN_addBoostOption(uint64 _period, uint64 _boostBps)
        external
    ...
    function ADMIN_setReferralCodeRegistry(
        ReferralCodeRegistry _referralCodeRegistry
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        referralCodeRegistry = _referralCodeRegistry;
    }

    function ADMIN_setDrxRewardPool(AutoRewardPool _drxRewardPool)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        drxRewardPool = _drxRewardPool;
    ...
    function ADMIN_setProjectDev(address _projectDev)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        projectDev = _projectDev;
    ...
    function ADMIN_setReferralRewardBps(uint256 _referralRewardBps)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function ADMIN_setBurnBps(uint256 _burnBps)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        burnBps = _burnBps;
